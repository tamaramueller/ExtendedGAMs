...
def get_cross_class_neighbourhood_similarity_discrete(edges, labels, one_hot_input:bool=False, train_mask=None):
    """
...
def get_cross_class_neighbourhood_similarity_continuous(adj_matrix, labels, one_hot:bool=False, train_mask=None):
    """
    calculate the cross-class neighbourhood similarity from https://arxiv.org/pdf/2106.06134.pdf
    """
    if train_mask is not None  and not train_mask.dtype == torch.bool:
...
def get_homophily_discrete(edges, nr_nodes, labels, train_mask=None):
    """
    calculate node homophily of a graph with a discrete adjacency matrix
    """
    if train_mask is not None and not train_mask.dtype == torch.bool:
        train_mask = train_mask.bool()
...
def get_homophily_continuous(weight_matrix, labels, train_mask=None):
    """
...
def get_homophily_discrete_regression(edges, nr_nodes, labels, train_mask=None):
    """
    calculate node homophily for regression tasks with discrete adjacency matrices (edges)
    """
    if train_mask is not None  and not train_mask.dtype == torch.bool:
        train_mask = train_mask.bool()
...
def get_homophily_continuous_regression(adj_matrix, labels, train_mask=None):
    """
    calculate the node homophily for regression tasks with continuous adjacency matrices 
    """
    if train_mask is not None  and not train_mask.dtype == torch.bool:
        train_mask = train_mask.bool()
